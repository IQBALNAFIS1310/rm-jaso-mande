...
export default function MenuManager() {
  ...
  const updateMenu = async (id) => {
    const { data } = await supabase.from("menus").update({ name: editName, price: editPrice }).eq("id", id).select();
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4">
        <input value={name} onChange={e => setName(e.target.value)} placeholder="Nama Menu" className="border p-2"/>
        <input type="number" value={price} onChange={e => setPrice(Number(e.target.value))} placeholder="Harga" className="border p-2"/>
        ...
        <tbody>
          {menus.map(menu => (
            <tr key={menu.id} className="border">
              {editId === menu.id ? (
                <>
                  <td className="border p-1"><input value={editName} onChange={e => setEditName(e.target.value)} className="border w-full p-1"/></td>
                  <td className="border p-1"><input type="number" value={editPrice} onChange={e => setEditPrice(Number(e.target.value))} className="border w-full p-1"/></td>
                ...
                <>
                  ...
                  <td className="border p-2 flex gap-1">
                    <button onClick={() => { setEditId(menu.id); setEditName(menu.name); setEditPrice(menu.price); }} className="bg-yellow-500 text-white px-2 rounded">Edit</button>
